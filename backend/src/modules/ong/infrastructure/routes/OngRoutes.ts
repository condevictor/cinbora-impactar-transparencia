...
async function ongRoutes(fastify: FastifyInstance) {
  fastify.get("/ongs", { preHandler: cacheMiddleware(300), schema: getNgosSchema }, ongController.getAll.bind(ongController));
  fastify.get("/ongs/:id", { preHandler: cacheMiddleware(180), schema: getNgoAndGraficSchema }, ongController.getOneWithGrafic.bind(ongController));
