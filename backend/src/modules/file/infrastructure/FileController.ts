...
import { UploadOngFileUseCase, UploadActionFileUseCase, DeleteFileUseCase, GetActionFilesByCategoryUseCase, GetOngFilesByCategoryUseCase } from "@modules/file";
...
class FileController {
  private uploadOngFileUseCase: UploadOngFileUseCase;
  private uploadActionFileUseCase: UploadActionFileUseCase;
  private deleteFileUseCase: DeleteFileUseCase;
  private getActionFilesByCategoryUseCase: GetActionFilesByCategoryUseCase;
  private getOngFilesByCategoryUseCase: GetOngFilesByCategoryUseCase;

  constructor() {
    const fileRepository = new FileRepository();
    this.uploadOngFileUseCase = new UploadOngFileUseCase(fileRepository);
    this.uploadActionFileUseCase = new UploadActionFileUseCase(fileRepository);
    this.deleteFileUseCase = new DeleteFileUseCase(fileRepository);
    this.getActionFilesByCategoryUseCase = new GetActionFilesByCategoryUseCase(fileRepository);
    this.getOngFilesByCategoryUseCase = new GetOngFilesByCategoryUseCase(fileRepository);
  }
  ...
  async uploadOngFile(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      const fileEntity = await this.uploadOngFileUseCase.execute(fileBuffer, filename, category, mimetype, size, request.user.ngoId);
  ...
  async uploadActionFile(request: FastifyRequest<{ Params: ActionParams }>, reply: FastifyReply) {
    ...
    try {
      const fileEntity = await this.uploadActionFileUseCase.execute(fileBuffer, filename, category, mimetype, size, request.params.actionId);
      reply.send({ fileUrl: fileEntity.aws_url });
    } catch (error) {
      console.error("Error uploading file:", error);
      reply.status(500).send({ error: "Error uploading file" });
  ...
  async getActionFilesByCategory(request: FastifyRequest<{ Params: ActionParams }>, reply: FastifyReply, category: string) {
    ...
    try {
      const files = await this.getActionFilesByCategoryUseCase.execute(actionId, category);
      reply.send(files);
    } catch (error) {
  ...
  async getOngFilesByCategory(request: FastifyRequest<{ Params: OngParams }>, reply: FastifyReply, category: string) {
    ...
    try {
      const files = await this.getOngFilesByCategoryUseCase.execute(ngoId, category);
      reply.send(files);
    } catch (error) {
      console.error(`Error fetching ${category} files:`, error);
      reply.status(500).send({ error: `Error fetching ${category} files` });
