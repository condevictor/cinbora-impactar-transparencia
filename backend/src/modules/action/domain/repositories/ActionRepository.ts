...
class ActionRepository {
  ...
  async updateActionExpensesGrafic(actionId: string, newExpense: Record<string, number>): Promise<NgoExpensesGrafic> {
    try {
      ...
      return {
        ...updatedGrafic,
        createdAt: updatedGrafic.createdAt.toISOString(),
        updatedAt: updatedGrafic.updatedAt.toISOString(),
        categorysExpenses: updatedExpenses as Record<string, number>[],
      };
    } catch (error) {
  ...
  async findExpensesByActionId(actionId: string): Promise<NgoExpensesGrafic | null> {
    try {
      return prismaClient.actionExpensesGrafic.findFirst({
        where: { actionId },
      }) as unknown as NgoExpensesGrafic;
    } catch (error) {
